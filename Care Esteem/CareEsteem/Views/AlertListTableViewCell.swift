...
class AlertListTableViewCell:UITableViewCell{
    ...
    func setupData(model:AlertModel){
        ...
        self.lbl_Time.font = UIFont(name: "RobotoSlab-Regular", size: 13)
...
//        self.lbl_Time.text = model.convertDateToString(date: createdAt ?? Date(), format: "dd/MM/yyyy 'at' hh:mm a")
        self.lbl_Time.text = convertDateToString(date: createdAt ?? Date(),
                                                 format: "dd/MM/yyyy hh:mm ")
        if model.isExpand ?? false {
            tableViewHeight.constant = CGFloat((model.bodyPartNames?.count ?? 0) * 300)
            self.imgDropDown.image = UIImage(named: "upArrow")
            self.detailView.isHidden = false
        } else {
            tableViewHeight.constant = 0
            self.imgDropDown.image = UIImage(named: "downArrow")
            self.detailView.isHidden = true
        }

//        if model.isExpand ?? false{
//            tableViewHeight.constant = CGFloat((model.bodyPartNames?.count ?? 0) * 300)
//            self.imgDropDown.image = UIImage(systemName: "upArrow")
//            self.detailView.isHidden = false
//        }else{
//            tableViewHeight.constant = 0
//            self.imgDropDown.image = UIImage(systemName: "downArrow")
//            self.detailView.isHidden = true
//        }
