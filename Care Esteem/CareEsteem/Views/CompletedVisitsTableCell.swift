...
class CompletedVisitsTableCell:UITableViewCell{
    ...
    // 🔹 Sort visits by actualStartTime
          func sortVisitsByStartTime(visits: [VisitsModel]) -> [VisitsModel] {
              return visits.sorted { v1, v2 in
                  let t1 = timeStringToDate(v1.actualStartTime?.first ?? "") ?? Date.distantFuture
                  let t2 = timeStringToDate(v2.actualStartTime?.first ?? "") ?? Date.distantFuture
                  return t1 < t2
              }
          }
