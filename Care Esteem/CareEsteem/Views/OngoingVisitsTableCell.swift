...
class OngoingVisitsTableCell:UITableViewCell{
    ...
    var clientobject : ClientModel?
    
    override func awakeFromNib() {
        super.awakeFromNib()
        self.lblTime.font = UIFont(name: "RobotoSlab-Regular", size: 11)
        invalidateTimer()
    ...
    //        formatter.locale = Locale(identifier: "en_GB") // en_GB uses 24h
    //
    ...
    private func updateVisitTime() {
        ...
        // ✅ Agar 24 hour se zyada ho gaya
        if totalSeconds >= 86400 { // 24 * 60 * 60
            lblTime.text = "Time's up!"
            invalidateTimer() // 🔥 Pehle wala logic wapas
            return
        }
        ...
        // ✅ Show total minutes:seconds
        lblTime.text = String(format: "%d:%02d", totalMinutes, seconds)
        
        // ✅ Enable checkout button after 2 minutes
        if totalMinutes >= 2 {
            btnCheckout.isUserInteractionEnabled = true
            btnCheckout.alpha = 1.0
            btnCheckout.backgroundColor = UIColor(named: "appRed")
