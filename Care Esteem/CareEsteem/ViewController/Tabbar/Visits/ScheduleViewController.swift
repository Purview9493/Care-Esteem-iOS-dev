...
class ScheduleViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var ViewToDoNoData: UIView!
    @IBOutlet weak var lbl_address2: UILabel!
    @IBOutlet weak var searchTodoHeightConstraints: NSLayoutConstraint!
    @IBOutlet weak var medicationSearchView: UIView!
    @IBOutlet weak var PNRMedicationHearderView: UIView!
    @IBOutlet weak var MedicationNoDataView: UIView!
    ...
    var noTodoNotesIV: UIImageView!
    var nomedicationIV:UIImageView!
    ...
    @IBAction func tapAddVisitNotsButton(_ sender: Any) {
    ...
    func textFieldoutLines(txtFld: UITextField) {
        ...
        let imagView = UIImageView(frame: CGRect(x: 11, y: 5, width: 15, height: 15))
        ...
        txtFld.leftView = bg
        txtFld.font = UIFont.robotoSlab(.regular, size: 14)
    }
    ...
    override func viewDidLoad() {
        ...
        noTodoNotesIV = self.view.viewWithTag(11) as? UIImageView
        let imageDataToDo = try! Data(contentsOf: Bundle.main.url(forResource: "todoNoData", withExtension: "gif")!)
        noTodoNotesIV.image = UIImage.sd_image(withGIFData: imageDataToDo)
        nomedicationIV = self.view.viewWithTag(12) as? UIImageView
        let imageDataMedication = try! Data(contentsOf: Bundle.main.url(forResource: "Bottle", withExtension: "gif")!)
        nomedicationIV.image = UIImage.sd_image(withGIFData: imageDataMedication)
        ...
        self.ViewToDoNoData.isHidden = true
        self.MedicationNoDataView.isHidden = true
    ...
    func setData() {
        ...
        if let clientPostcode = self.visit?.clientPostcode, let clientCity = self.visit?.clientCity{
            self.lbl_address2.text =  clientCity + ", " + clientPostcode
        }
        //            if visitType != .onging {
        ...
        if self.visitType == .completed{
            ...
            self.btnCheckout.backgroundColor = UIColor(named: "appOffGray")
    ...
    //
    //        if totalSeconds >= twentyFourHours {
    //            self.lblTime.text = "Time's up"
    //            self.btnCheckout.isUserInteractionEnabled = false
    //            self.btnCheckout.backgroundColor = .gray
    //            self.btnCheckout.setTitleColor(.black, for: .normal)
    ...
    //        dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
    //        dateFormatter.timeZone = TimeZone(identifier: "Europe/London")
    //        dateFormatter.locale = Locale(identifier: "en_GB") // Force 24-hour format
    //
    //        guard let startDate = dateFormatter.date(from: fullDateTimeStr) else {
    //            self.lblTime.text = "00:00"
    //            self.btnCheckout.isUserInteractionEnabled = false
    //            self.btnCheckout.alpha = 0.5
    //            self.btnCheckout.backgroundColor = UIColor.lightGray
    ...
    //
    //        // 🔥 Update label (MM:SS if < 1 hour, otherwise HH:MM:SS)
    //        if hours > 0 {
    //            self.lblTime.text = String(format: "%02d:%02d:%02d", hours, minutes, seconds)
    //        } else {
    //            self.lblTime.text = String(format: "%02d:%02d", minutes, seconds)
    ...
    @IBAction func tapAddVisitNoteButton(_ sender: Any) {
        if self.visitType != .onging{
            var style = ToastStyle()
            style.backgroundColor = .red
            self.view.makeToast("Changes are not allowed", duration: 2.0, position: .bottom, style: style)
    ...
    func setupSelected(view:UIButton){
        view.backgroundColor = UIColor(named: "appGreen")
        view.setTitleColor(.white, for: .normal)
    }
    func setupUnselected(view:UIButton){
    ...
    //            cell.lblStatus.text = model.todoOutcome
                ...
                if (data.medicationType == "Scheduled") {
                    if(data.selectPreference?.lowercased() == "by exact time") {
                        ...
                        cell.lbltype.text = cell.lbltype.text! +  "\n@ N/A"
                        cell.popupImageWidthConstraints.constant = 20
                    }
                    
                }
                if (data.medicationType == "PRN") {
                    cell.popupImageWidthConstraints.constant = 20
                    cell.lbltype.text = cell.lbltype.text! +  "\n\(data.dosePer ?? 0) Doses per \(data.doses ?? 0) \(data.timeFrame ?? "")"
...
//                    cell.lbltype.text = cell.lbltype.text! +  "\n\(data.doses ?? 0) Doses per \(data.dosePer ?? 0) \(data.timeFrame ?? "")"
                ...
                if (array[indexPath.row].status ?? "") == ""{
                    cell.viewStatus.isHidden = true
                } else if (array[indexPath.row].status ?? "") == "Fully Taken"{
                    cell.viewStatus.isHidden = false
                    cell.viewStatus.backgroundColor = UIColor(named: "appGreen") ?? .green
                    cell.constantviewhighcomplet.constant = 25
                    ...
                    cell.constantviewhighcomplet.constant = 33
                } else {
                    cell.viewStatus.isHidden = true
                }
                ...
                cell.applyCardStyle()
                cell.lblName.text =
                    self.pnrMedicationList[indexPath.row].nhsMedicineName
                cell.lblStatus.text = self.pnrMedicationList[indexPath.row].status
                cell.lbldesc.text = self.pnrMedicationList[indexPath.row].medicationSupport
                ...
                if (self.pnrMedicationList[indexPath.row].status ?? "") == ""{
                    cell.viewStatus.isHidden = true
                }else if (self.pnrMedicationList[indexPath.row].status ?? "") == "Fully Taken"{
                    cell.viewStatus.isHidden = false
                    cell.viewStatus.backgroundColor = UIColor(named: "appGreen") ?? .green
    ...
    //            if model.medicationType == "PRN" {
                ...
                cell.lbltype.text =  "PRN \n\(self.pnrMedicationList[indexPath.row].dosePer ?? 0) Doses per \(self.pnrMedicationList[indexPath.row].doses ?? 0) \(self.pnrMedicationList[indexPath.row].timeFrame ?? "")"
...
//                    cell.lbltype.text =  "PRN \n\(self.pnrMedicationList[indexPath.row].doses ?? 0) Doses per \(self.pnrMedicationList[indexPath.row].dosePer ?? 0) \(self.pnrMedicationList[indexPath.row].timeFrame ?? "")"
    ...
    //            }
                ...
                cell.setupData(model: self.visitNotesList[indexPath.row],visitType: visitType)
                cell.clickHandler = {
                    let vc = Storyboard.Visits.instantiateViewController(withViewClass: AddEditVisitNotesViewController.self)
                    vc.visitDetaiID = (self.visit?.visitDetailsID ?? "").description
                    vc.isEdit = true
...
//                            self?.searchTodoHeightConstraints.constant = 40
//                            self?.ViewToDoNoData.isHidden = true
...
//                        }else{
//                            self?.searchTodoHeightConstraints.constant = 0
//                            self?.ViewToDoNoData.isHidden = false
//                            self?.todoTableView.isHidden = true
//                         //   self?.view.makeToast(data.message ?? "")
//    //                        self?.view.showToastwithImage(message: data.message ?? "", image: UIImage(named: "acceptedIcon"), textColor: .white, backgroundColor: .red)
//                        }
//                    }
//                case .failure(let error):
//                    DispatchQueue.main.async {[weak self] in
...
//        }
        ...
        //
        //                        self?.lblTodoCount.text = "Essentials : \(count?.count ?? 0)/\(self?.todoList.count ?? 0)"
                                // ✅ Filter only essential todos
                                let essentialTodos = self?.todoList.filter { $0.todoEssential == true }
                                let completedEssentials = essentialTodos?.filter { !($0.todoOutcome ?? "").isEmpty }
                                           
                                if ((essentialTodos?.isEmpty) != nil) {
                                               // ❌ No essential todos → hide label
                                               self?.lblTodoCount.isHidden = false
                                    self?.lblTodoCount.text = "Essentials : \(completedEssentials?.count ?? 0)/\(essentialTodos?.count ?? 0)"
                                           } else {
                                               // ✅ Show only essential todos progress
                                               self?.lblTodoCount.isHidden = true
                ...
                case .success(let data):
                    DispatchQueue.main.async { [weak self] in
                        if data.statusCode == 200{
                            ...
                            self?.tf.isHidden = self?.pnrMedicationList.isEmpty ?? true
                          ...
                          //  self?.lblMedicationCount.text = "Completed : \(completeCount.count)/\(data.data?.count ?? 0)"
                            self?.lblMedicationCount.text = "Completed : \(completeCount.count)/\(self?.medicationList.count ?? 0)"
                          //  self.medicationList.count

                            // Reload tables
                            self?.MedicationNoDataView.isHidden = true
                            self?.PNRMedicationHearderView.isHidden = false
                            self?.medicationSearchView.isHidden = false
                            self?.medicationTableView.reloadData()
                            self?.pnrMedicationTableView.reloadData()
                        }else{
                            self?.MedicationNoDataView.isHidden = false
                            self?.PNRMedicationHearderView.isHidden = true
                            self?.medicationSearchView.isHidden = true
                          //  self?.view.makeToast(data.message ?? "")
                        }
                    }
                case .failure(let error):
        ...
        private func getVisitNoteList_APICall() {
                ...
                WebServiceManager.sharedInstance.callAPI(
                    apiPath: .getVisitNotesDetails(visitId: (self.visit?.visitDetailsID ?? "").description),
                    method: .get,
                    params: [:],
                    isAuthenticate: true,
                    ...
                    case .success(let data):
                        DispatchQueue.main.async { [weak self] in
                            ...
                            if data.statusCode == 200 {
                                ...
                                if self.visitNotesList.count == 0 {
                                    
                                    self.StackView.isHidden = true
                                    self.visitNoteTableView.isHidden = true
                                    self.viewVisitNotesNoData.isHidden = false
                                } else {
                                    self.StackView.isHidden = visitType == .completed ? true : false
