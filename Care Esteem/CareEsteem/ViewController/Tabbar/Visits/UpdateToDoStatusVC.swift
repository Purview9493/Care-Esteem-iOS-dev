...
class UpdateToDoStatusVC: UIViewController {
    ...
    @IBOutlet weak var lbl_todoName: UILabel!
    ...
    override func viewDidLoad() {
        ...
        self.lbl_todoName.text = self.todo?.todoName
...
extension UpdateToDoStatusVC {
    ...
    private func AddAlert_APICall() {
        let todoDetailsID = self.todo?.todoDetailsID ?? ""
        let visitDetailsID = self.visit?.visitDetailsID ?? ""
        var params = [APIParameters.ToDo.clientId: self.visit?.clientID ?? 0,
                      ...
                      APIParameters.ToDo.todo_details_id: todoDetailsID,
                      APIParameters.ToDo.visitDetailsId: visitDetailsID,
