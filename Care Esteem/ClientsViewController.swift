...
class ClientsViewController: UIViewController, UITextFieldDelegate {
    ...
    private func setupSearchTextField() {
        ...
        tf.font = UIFont.robotoSlab(.regular, size: 12)
        let imagView = UIImageView(frame: CGRect(x: 12, y: 9, width: 14, height: 14))
...
extension ClientsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView,
        ...
        let cell = tableView.dequeueReusableCell(withClassIdentifier: ClientListTableViewCell.self,for: indexPath)
        let model = searchActive ? filterList[indexPath.section] : list[indexPath.section]
        cell.setupData(model: model)
    ...
    func tableView(_ tableView: UITableView,didSelectRowAt indexPath: IndexPath) {
        let vc = Storyboard.Clients.instantiateViewController(withViewClass: ClientDetail2VC.self)
        vc.client = searchActive ? filterList[indexPath.section] : list[indexPath.section]
                ...
                DispatchQueue.main.async {
                    if data.statusCode == 200{
                        ...
                        NotificationCenter.default.post(
                            ...
                            object: nil,
                            userInfo: ["message": "Data from the sender!", "count": array.count]
                        )
                    }
                }
            case .failure(_):
...
extension ClientsViewController {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        btn_cancel.isHidden = trimmedText.isEmpty
        searchActive = !trimmedText.isEmpty
        
        if searchActive {
            filterList = list.filter { $0.fullName?.lowercased().contains(trimmedText.lowercased()) ?? false }
        } else {
            filterList.removeAll()
        }
...
//           tf.leftViewMode = .always
//           
//           let imagView = UIImageView(frame: CGRect(x: 12, y: 7, width: 14, height: 14))
//           imagView.image = UIImage(named: "searchIcon")
//           let bg = UIView(frame: CGRect(x: 0, y: 0, width: 28, height: 28))
//           bg.addSubview(imagView)
//           tf.leftView = bg
